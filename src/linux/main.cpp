...
void ae_callback(int position, void*) {
    ...
    cout << "\n\nget ae status:"<< ((get_AE_status() == 1) ? "enable" : "disable") << endl;// 1: enable; 0: disable
}
...
void awb_callback(int position, void*) {
    ...
    cout << "\n\nget awb status:"<< ((get_AWB_status() == 1) ? "enable" : "disable") << endl;// 1: enable; 0: disable
}
        ...
        while(true) {
            switch (item_input) {
                ...
                case 'n':
                    uint16_t GetZNear;
                    uint16_t GetZFar;
                    getDepthOfField(&GetZNear, &GetZFar);
...
void point_cloud_view_with_opengl() {
    ...
    //prepare color, depth frame
    int color_size = config.colorWidth * config.colorHeight;
    BYTE *color_frame = new BYTE[color_size * 3];
    BYTE *color_frame_out = new BYTE[color_size * 3];
    cout << "\ncolor_frame size: " << color_size << endl;
    int depth_size = config.colorWidth * config.colorHeight; // raw data
    BYTE *depth_frame = new BYTE[depth_size * 2];
    float *depth_frame_out = new float[depth_size * 3];
    cout << "\nget_depth_handler size: " << depth_size << endl;
